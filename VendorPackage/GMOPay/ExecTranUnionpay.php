...
if( isset( $_POST['submit'] ) ){
	...
	$input = new ExecTranUnionpayInput();/* @var $input ExecTranUnionpayInput */
	...
	$input->setCommodityName($_POST['CommodityName']);
	...
	//API通信クラスをインスタンス化します
	$exe = new ExecTranUnionpay();/* @var $exec ExecTranUnionpay */
	
	//パラメータオブジェクトを引数に、実行メソッドを呼び、結果を受け取ります。
	$output = $exe->exec( $input );/* @var $output ExecTranUnionpayOutput */
...
//ExecTranUnionpay入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/ExecTranUnionpay.php' );