...
if( isset( $_POST['submit'] ) ){
	require_once( 'com/gmo_pg/client/input/RakutenIdChangeInput.php');
	require_once( 'com/gmo_pg/client/tran/RakutenIdChange.php');
	
	//入力パラメータクラスをインスタンス化します
	$input = new RakutenIdChangeInput();/* @var $input RakutenIdChangeInput */
	...
	$input->setUseCoupon($_POST['UseCoupon']);

	
	//API通信クラスをインスタンス化します
	$exe = new RakutenIdChange();/* @var $exec RakutenIdChange */
	
	//パラメータオブジェクトを引数に、実行メソッドを呼び、結果を受け取ります。
	$output = $exe->exec( $input );/* @var $output RakutenIdChangeOutput */
...
//RakutenIdChange入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/RakutenIdChange.php' );