...
if( isset( $_POST['submit'] ) ){
	require_once( 'com/gmo_pg/client/input/LinepaySalesInput.php');
	require_once( 'com/gmo_pg/client/tran/LinepaySales.php');
	
	//入力パラメータクラスをインスタンス化します
	$input = new LinepaySalesInput();/* @var $input LinepaySalesInput */
	...
	//API通信クラスをインスタンス化します
	$exe = new LinepaySales();/* @var $exec LinepaySales */
	
	//パラメータオブジェクトを引数に、実行メソッドを呼び、結果を受け取ります。
	$output = $exe->exec( $input );/* @var $output LinepaySalesOutput */
...
//LinepaySales入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/LinepaySales.php' );