...
if( isset( $_POST['submit'] ) ){
	...
	$input = new AlterTranMagstripeInput();/* @var $input AlterTranMagstripeInput */
	...
	//API通信クラスをインスタンス化します
	$exe = new AlterTranMagstripe();/* @var $exec AlterTranMagstripe */
	
	//パラメータオブジェクトを引数に、実行メソッドを呼び、結果を受け取ります。
	$output = $exe->exec( $input );/* @var $output AlterTranMagstripeOutput */
...
//AlterTranMagstripe入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/AlterTranMagstripe.php' );