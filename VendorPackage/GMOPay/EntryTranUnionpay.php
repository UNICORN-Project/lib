...
if( isset( $_POST['submit'] ) ){
	...
	$input = new EntryTranUnionpayInput();/* @var $input EntryTranUnionpayInput */
	...
	//API通信クラスをインスタンス化します
	$exe = new EntryTranUnionpay();/* @var $exec EntryTranUnionpay */
	
	//パラメータオブジェクトを引数に、実行メソッドを呼び、結果を受け取ります。
	$output = $exe->exec( $input );/* @var $output EntryTranUnionpayOutput */
...
//EntryTranUnionpay入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/EntryTranUnionpay.php' );