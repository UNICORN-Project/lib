...
if( isset( $_POST['submit'] ) ){
	require_once( 'com/gmo_pg/client/input/RecruitUpdateAuthInput.php');
	require_once( 'com/gmo_pg/client/tran/RecruitUpdateAuth.php');
	
	//入力パラメータクラスをインスタンス化します
	$input = new RecruitUpdateAuthInput();/* @var $input RecruitUpdateAuthInput */
	...
	//API通信クラスをインスタンス化します
	$exe = new RecruitUpdateAuth();/* @var $exec RecruitUpdateAuth */
	
	//パラメータオブジェクトを引数に、実行メソッドを呼び、結果を受け取ります。
	$output = $exe->exec( $input );/* @var $output RecruitUpdateAuthOutput */
...
//RecruitUpdateAuth入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/RecruitUpdateAuth.php' );