...
if( isset( $_POST['submit'] ) ){
	require_once( 'com/gmo_pg/client/input/RecruitChangeInput.php');
	require_once( 'com/gmo_pg/client/tran/RecruitChange.php');
	
	//入力パラメータクラスをインスタンス化します
	$input = new RecruitChangeInput();/* @var $input RecruitChangeInput */
	...
	//API通信クラスをインスタンス化します
	$exe = new RecruitChange();/* @var $exec RecruitChange */
	
	//パラメータオブジェクトを引数に、実行メソッドを呼び、結果を受け取ります。
	$output = $exe->exec( $input );/* @var $output RecruitChangeOutput */
...
//RecruitChange入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/RecruitChange.php' );