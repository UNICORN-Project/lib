...
if( isset( $_POST['submit'] ) ){
	...
	$input = new ExecTranMagstripeInput();/* @var $input ExecTranMagstripeInput */
	...
	//API通信クラスをインスタンス化します
	$exe = new ExecTranMagstripe();/* @var $exec ExecTranMagstripe */
	
	//パラメータオブジェクトを引数に、実行メソッドを呼び、結果を受け取ります。
	$output = $exe->exec( $input );/* @var $output ExecTranMagstripeOutput */
...
//ExecTranMagstripe入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/ExecTranMagstripe.php' );