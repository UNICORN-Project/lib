...
if( isset( $_POST['submit'] ) ){
	require_once( 'com/gmo_pg/client/input/UnionpaySalesInput.php');
	require_once( 'com/gmo_pg/client/tran/UnionpaySales.php');
	
	//入力パラメータクラスをインスタンス化します
	$input = new UnionpaySalesInput();/* @var $input UnionpaySalesInput */
	...
	//API通信クラスをインスタンス化します
	$exe = new UnionpaySales();/* @var $exec UnionpaySales */
	
	//パラメータオブジェクトを引数に、実行メソッドを呼び、結果を受け取ります。
	$output = $exe->exec( $input );/* @var $output UnionpaySalesOutput */
...
//UnionpaySales入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/UnionpaySales.php' );