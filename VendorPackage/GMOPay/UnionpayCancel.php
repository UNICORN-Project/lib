...
if( isset( $_POST['submit'] ) ){
	require_once( 'com/gmo_pg/client/input/UnionpayCancelInput.php');
	require_once( 'com/gmo_pg/client/tran/UnionpayCancel.php');
	
	//入力パラメータクラスをインスタンス化します
	$input = new UnionpayCancelInput();/* @var $input UnionpayCancelInput */
	...
	//API通信クラスをインスタンス化します
	$exe = new UnionpayCancel();/* @var $exec UnionpayCancel */
	
	//パラメータオブジェクトを引数に、実行メソッドを呼び、結果を受け取ります。
	$output = $exe->exec( $input );/* @var $output UnionpayCancelOutput */
...
//UnionpayCancel入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/UnionpayCancel.php' );