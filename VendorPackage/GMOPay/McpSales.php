...
if( isset( $_POST['submit'] ) ){
	require_once( 'com/gmo_pg/client/input/McpSalesInput.php');
	require_once( 'com/gmo_pg/client/tran/McpSales.php');
	
	//入力パラメータクラスをインスタンス化します
	$input = new McpSalesInput();/* @var $input McpSalesInput */
	...
	//API通信クラスをインスタンス化します
	$exe = new McpSales();/* @var $exec McpSales */
	
	//パラメータオブジェクトを引数に、実行メソッドを呼び、結果を受け取ります。
	$output = $exe->exec( $input );/* @var $output McpSalesOutput */
...
//McpSales入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/McpSales.php' );