...
if( isset( $_POST['submit'] ) ){
	require_once( 'com/gmo_pg/client/input/RecruitSalesInput.php');
	require_once( 'com/gmo_pg/client/tran/RecruitSales.php');
	
	//入力パラメータクラスをインスタンス化します
	$input = new RecruitSalesInput();/* @var $input RecruitSalesInput */
	...
	//API通信クラスをインスタンス化します
	$exe = new RecruitSales();/* @var $exec RecruitSales */
	
	//パラメータオブジェクトを引数に、実行メソッドを呼び、結果を受け取ります。
	$output = $exe->exec( $input );/* @var $output RecruitSalesOutput */
...
//RecruitSales入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/RecruitSales.php' );