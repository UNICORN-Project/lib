...
class RedirectUtil {
	 ...
	 * @reutnr string 雛形htmlの文字列
	 */
	function fletsStart($pagePath, $param, $encode = null) {
		$accessID = $param->getAccessID();
		$token = $param->getToken();
		...
		if (
			...
			$this->exception =
				new GPayException("必須フレッツ手続き開始パラメータに値が入っていません。", $this->exception);
			return null;
		}
		...
		$urlMap = new ConnectUrlMap();
		$url = $urlMap->getUrl('fletsStart');
		// 雛形ページ中のパラメータ項目を置き換え
		$strPage = str_replace('${FletsStartURL}', $url, $strPage);
		$strPage = str_replace('${AccessID}', $accessID, $strPage);
		$strPage = str_replace('${Token}', $token, $strPage);
