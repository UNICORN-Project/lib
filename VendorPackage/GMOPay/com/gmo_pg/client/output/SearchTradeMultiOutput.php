...
class SearchTradeMultiOutput extends BaseOutput {
	...
	/**
	 * @var string Flets 処理依頼通番
	 */
	var $requestNo;
	/**
	 * @var string Flets 処理受付番号
	 */
	var $accountNo;
	/**
	 * @var string Flets センターコード
	 */
	var $centerCode;
	/**
	 * @var integer Flets 変更前利用金額
	 */
	var $previousAmount;
	/**
	 * @var integer Flets 変更前税送料
	 */
	var $previousTax;
	/**
	...
	function __construct($params = null) {
        ...
        $this->setProcessDate($params->get('ProcessDate'));
       	...
       	$tax = $params->get('Tax');
       	$useBigDecimalAmount = $this->useBigDecimalAmount($amount);
		if ($useBigDecimalAmount === false) {
			$this->setAmount(is_numeric($amount) ? $amount : null);
            $this->setTax(is_numeric($tax) ? $tax : null);
		} else {
			$this->setAmountBigDecimal(is_numeric($tmp) ? $tmp : null);
        	$this->setTaxBigDecimal(is_numeric($tmp) ? $tmp : null);
		}
		...
		$this->setRequestNo($params->get('RequestNo'));
		$this->setAccountNo($params->get('AccountNo'));
		$this->setCenterCode($params->get('CenterCode'));
	...
	function useBigDecimalAmount($payType) {
		...
		$list = str_getcsv(USE_BIG_DECIMAL_AMOUNT_PAY_TYPES);
	...
	function toString() {
		...
		$str .= '&';
		$str .= 'JibunReceiptNo=' . $this->$this->getJibunReceiptNo();
		...
		$str .= '&';
		$str .= 'RequestNo=' . $this->getRequestNo();
		$str .= '&';
		$str .= 'AccountNo=' . $this->getAccountNo();
		$str .= '&';
		$str .= 'CenterCode=' . $this->getCenterCode();
		$str .= '&';
		$str .= 'PreviousAmount=' . $this->getPreviousAmount();
		$str .= '&';
		$str .= 'PreviousTax=' . $this->getPreviousTax();
		$str .= '&';
