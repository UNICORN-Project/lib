...
class BaseTran {
	...
	function connect($url) {
		...
		// SSL認証の設定
		curl_setopt($urlConnect, CURLOPT_SSL_VERIFYHOST, false);
		// サーバ証明書の検証を行わない
		...
		curl_setopt($urlConnect, CURLOPT_SSLVERSION, CURL_SSLVERSION_TLSv1);
		//curl_setopt($urlConnect, CURLOPT_SSL_CIPHER_LIST, 'rsa_rc4_128_sha');
	...
	function sendData(&$urlConnect, $params) {
		...
		if (false == $retData) {
			...
			$curl_errorno = curl_errno($urlConnect);
	...
	function callProtocol_($url, $params) {
		...
		if (mb_strpos($url, "ExecTran.idPass") === false
