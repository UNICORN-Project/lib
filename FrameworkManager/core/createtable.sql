...
-- テーブルの構造 `session`
CREATE TABLE IF NOT EXISTS `session` (`token` varchar(255) NOT NULL COMMENT 'ワンタイムトークン', `created` datetime NOT NULL DEFAULT '1000-01-01 00:00:00' COMMENT 'トークン作成日時', PRIMARY KEY (`token`)) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='セッションテーブル';
-- テーブルの構造 `sessiondata`
CREATE TABLE IF NOT EXISTS `sessiondata` (`identifier` varchar(96) NOT NULL COMMENT 'deviceテーブルのPkey', `data` text COMMENT 'jsonシリアライズされたセッションデータ', `modified` datetime NOT NULL DEFAULT '1000-01-01 00:00:00' COMMENT '変更日時', PRIMARY KEY (`identifier`)) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='セッションデータテーブル';
-- Default user record
