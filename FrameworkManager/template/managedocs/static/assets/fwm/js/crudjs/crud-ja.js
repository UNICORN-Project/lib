...
function crud(baseCRUDURL, baseQuery, callback) {
	...
	if ("" != table) {
		...
		$.ajax({
			...
			cache : false,
			error : function (data){
				$("#crudmain h2").text("Error :" + data.status + " " + data.statusText);
			},
						...
						$("#crudmain").html(html).ready(function() {
							...
							$(".crudlink").each(function() {
								...
								$(".list-paginglink").each(function(){
									...
									// 新規レコード作成リンク
									$("#crudmain").append("<div class=\"create-new-record-link\"><a href=\"" + location.protocol + "//" + location.hostname + location.pathname + "?" + baseQuery + "&mode=new&table="+table+"&limit="+limit+"&offset="+offset+"&ORDER="+encodeURIComponent(order)+"&LIKE="+encodeURIComponent(like) + "\">create new record</a></div>").ready(function() {
										if(typeof callback != "undefined"){
											callback("list");
										}
									});
