...
- (void)viewDidLoad
{
    ...
    if (YES != firstViewed){
        firstViewed = YES;
        // ATSのチェック
        NSDictionary *appTransportSecurityDic = [[[NSBundle mainBundle] infoDictionary] objectForKey:@"NSAppTransportSecurity"];
        if (nil != appTransportSecurityDic){
            BOOL allowsArbitraryLoads = (BOOL)[appTransportSecurityDic objectForKey:@"NSAllowsArbitraryLoads"];
            if (YES == allowsArbitraryLoads){
                [MMessages showMessage:@"999" :NSLocalizedString(@"Warrning\rthis App's 'ATS' unenabled.", @"Warring\rこのアプリケーションは'ATS'が無効になっています。") :MMessageWarrning :nil completion:^(NSString *messageIdentifier) {
