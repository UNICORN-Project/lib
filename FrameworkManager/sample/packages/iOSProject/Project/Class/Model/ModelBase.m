...
- (NSString *)createToken;
{
    ...
    NSString *identifier = [ModelBase loadIdentifier:cryptKey :cryptIV];
    if (nil == identifier || ![identifier isKindOfClass:NSClassFromString(@"NSString")] || !(1 < identifier.length)){
        // UUIDの初期化
        identifier = [SecureUDID UDIDForDomain:domain usingKey:cryptKey];
