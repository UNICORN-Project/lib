...
public class AsyncHttpClientAgent {
	...
	public static void put(final Context context, final String url, RequestParams params,
			final JsonHttpResponseHandler responseHandler) {
		if (params == null) {
			params = new RequestParams();
		}
		...
		asyncHttpClient.setUserAgent(getUserAgent(context));
		asyncHttpClient.addHeader("Content-Type", "text/html; charset=UTF-8");
		asyncHttpClient.put(url, finalParams, responseHandler);
	}

	public static void postBinary(final Context context, final String url,
			HashMap<String, Object> argSaveParams, byte[] argUploadData, String argUploadDataName,
			String argUploadDataContentType, String argUploadDataKey,
			final JsonHttpResponseHandler responseHandler) {
		...
		builder.addBinaryBody(argUploadDataKey, argUploadData,
				ContentType.create(argUploadDataContentType), argUploadDataName);

		HttpEntity entity = builder.build();
		String contentType = null;
		asyncHttpClient.post(context, url, null, entity, contentType, responseHandler);
	}
	...
	public static void putBinary(Context context, String url,
		...
		String contentType = null;
		asyncHttpClient.put(context, url, headers, ben, contentType, responseHandler);
	}

	public static void putBinary(Context context, String url, byte[] argUploadData,
			final JsonHttpResponseHandler responseHandler) {
	...
	public static void get(final Context context, final String url, RequestParams params,
		...
		asyncHttpClient.setUserAgent(getUserAgent(context));
		asyncHttpClient.addHeader("Content-Type", "text/html; charset=UTF-8");
		asyncHttpClient.get(url, finalParams, responseHandler);
	}
	...
	public static CookieStore loadCookies(Context context, DefaultHttpClient defaultHttpClient) {
		...
		for (Cookie cookie : cookies) {
			if (cookie.getDomain().equals(Constant.DOMAIN_NAME)
					&& cookie.getName().equals(TOKEN_NAME)) {
		...
		if (!loginCookieEnabled) {
			...
			BasicClientCookie newCookie = new BasicClientCookie(TOKEN_NAME, token);
	...
	private static CookieStore createToken(Context context, DefaultHttpClient httpClient) {
		...
		String gmtstringdata = DateUtilities.nowToYyyyMMddHHmmsss();

		String identifier = UUIDCreate.get(context);
	...
	/**
	 * <p>
	 * unicorn/[versionName][versionCode]/[versionName] (Android; [ModelName] [OSVersion])
	 * </p>
	 * 
	 * @param context
	 * @return ユーザーエージェント文字列
	 */
	...
	public static Cookie getToken(Context context) {
		final AsyncHttpClient asyncHttpClient = new AsyncHttpClient();
		CookieStore cookieStore = AsyncHttpClientAgent.createToken(context,
				asyncHttpClient.getHttpClient());
		// CookieStore cookieStore = asyncHttpClient.getHttpClient().getCookieStore();
