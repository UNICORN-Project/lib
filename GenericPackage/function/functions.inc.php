...
function parse_phpinput_str(){
	...
	// バウンダリーがあるかどうか（multipartなPUTかどうか）
	if(!preg_match('/------------[a-zA-Z0-9]+\\r\\nContent-Disposition: form-data; name=".+"; filename=".+"\\r\\nContent-Type:/', $raw_data)){
		// プレーンな場合はparse_str
		logging('parse_str', 'post');
		parse_str($raw_data, $data);
	}
	else {
		logging('parse_phpinput_str', 'post');
		// バウンダリー毎に振り分ける
		...
		foreach ($parts as $part) {
			...
			if (isset($headers['content-disposition'])) {
				...
				switch ($name) {
					...
					default:
						$d = substr($body, 0, strlen($body) - 2);
						if (isset($filename)) {
							$data[$name] = $d;
						} else {
							$data[$name] = rawurldecode($d);
						}
						break;
				}
			}
